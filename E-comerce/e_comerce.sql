-- CONCLUIDO EM 29/11/2023

-- DB de E-comerce - Curso SGDB Specialist - DIO
-- criado em 26/11/2023 por Carlos Eduardo de Oliveira Franco
-- criação do banco de dados para o cenário de E-comerce

-- Descrição inicial
-- Tabelas:
-- Terceiro - vendedor
-- Terceiro-vendedor_has_produtos
-- Fornecedor
...
constraint fk_product_enterprise foreign key (EpId) references enterprise (EpId)
);
...
PrQtt int not null,
WhId int not null,
constraint fk_product_stock foreign key (PrId) references product (PrId),
constraint fk_product_warehouse foreign key (WhId) references warehouse (WhId)
);
...
constraint fk_costumer_users foreign key (UsID) references users(UsId)
);
...
CsId int not null,
constraint fk_costumer_chart foreign key (CsId) references costumer(CsId)
);
...
PrId int,
PrQtt int not null,
constraint fk__chart_chartcontent foreign key (ChId) references chart(ChId),
constraint fk_product_chart foreign key (PrId) references product(PrId)
);
...
constraint fk_chart_adress foreign key (AdId) references address(AdId),
constraint fk_chart_orders foreign key (ChId) references chart(ChId)
);
...
DeDt date,
constraint fk_delivery_OrId foreign key (OrId) references orders(OrId)
);
...
constraint fk_cards_users foreign key (UsId) references users(UsId)
);
...
constraint fk_payment_orders foreign key (OrId) references orders(OrId),
constraint fk_payment_cards foreign key (CdId) references cards(CdId)
);
...
EpId int,
UsId int,
constraint fk_seller_enterprise foreign key (EpId) references enterprise(EpId),
constraint fk_seller_users foreign key (UsId) references users(UsId)
);
...
select * from cards;
select * from chart;
select * from chartContent;
select * from orders;
select * from delivery;
select * from seller;


-- mostre todos os produtos por ordem de nome descendente
select * from product order by PrName desc;

select distinct ChId,PrName,PrQtt,PrValue,round(h.PrQtt*p.PrValue,2)
from product p, chartContent h where h.ChId=1;


-- mostre todos os usuários e seus endereços
select UsName,AdStreet,AdNumber,AdNeighborhood,AdCity,AdState from
users as u, address as a where
u.AdId=a.AdId  ORDER BY AdStreet;

-- mostre todos os usuários e seus cartões
select UsName,CdNum,CdName,CdValid from
users as u, cards as c where
c.UsId=u.UsId ORDER BY CdNum;

-- mostre todos os carrinhos de um usuário
select UsName,ChId
from users as u, chart as c, costumer as ct
where ct.UsId=u.UsId and ct.CsId=1;

-- mostrar detalhes do pedido para do carrinho 1
select c.ChId,p.PrId,p.PrName,p.PrQtt,p.PrValue from chart c, orders o
where c.ChId=o.ChId and c.ChId=1;

-- mostrar detalhes do fornecedor 1
select * from enterprise e, address a where
e.AdId = a.AdId and e.EpId=1;

-- mostrar detalhes do vendedor 1
select s.EpId, EpName, AdStreet from seller s, enterprise e, address a where
s.EpId=e.EpId and e.AdId = a.AdId and s.EpId=1;

-- mostrar detalhes do vendedor 1
select s.UsId, UsName, AdStreet from seller s, users u, address a where
s.UsId=u.UsId and u.AdId = a.AdId and s.UsId=1;

-- mostrar estoque nos depósitos
select distinct concat(w.WhName,' - ',a.AdCity) as deposito, p.PrName,p.PrDetails,s.PrQtt
from product p join warehouse w join address a join stock s where
p.PrId=s.PrId and w.WhId=s.WhId and w.AdId=a.AdId;
